...
class ContestAdapter (private val contestList: List<ContestData>): RecyclerView.Adapter<RecyclerView.ViewHolder>(){

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        val itemView = LayoutInflater.from(parent.context).inflate(R.layout.contest_position, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        holder.weekTitle.text = Constants.CURRENT_WEEK_CONTEST

        if(currentItem.podium?.first != null){
            holder.firstWrapper.alpha = 1F
